...
describe("A suite", function () {
  ...
  it("should recognize label exists properly", function () {
    ...
    expect(wrapper.find(sel("val-0")).at(0).text()).toEqual(
      expect.stringContaining("label1")
    );
    //expect(wrapper.exists(sel('val-0-0'))).toEqual(true);
    expect(wrapper.find(sel("val-0")).find(".description").text()).toEqual(
      "third level:\nlabel1"
    );

    expect(wrapper.find(sel("val-1")).at(0).text()).toEqual(
      expect.stringContaining("label2")
    );
    expect(wrapper.find(sel("val-1")).find(".description").text()).toEqual(
      "fourth level:\nlabel2"
    );

    expect(wrapper.find(sel("val-2")).at(0).text()).toEqual(
      expect.stringContaining("label3")
    );
    expect(wrapper.find(sel("val-2")).find(".description").text()).toEqual(
      "fifth level:\nlabel3"
    );
  ...
  it("should calculate label values properly", function () {
    ...
    expect(
      wrapper.find(sel("val-0")).find(".description").at(0).text()
    ).toEqual("third level:\nlabel:10.5");
    expect(
      wrapper.find(sel("val-0")).find(".description").at(1).text()
    ).toEqual("fourth level:\nlabel:20.5");
    expect(
      wrapper.find(sel("val-0")).find(".description").at(2).text()
    ).toEqual("fifth level:\nlabel:30.5");
