...
describe('A suite', function() {
    ...
    it('should edit value properly', function(){
        const wrapper=mount(<Map />);
        wrapper.find('.Leaf').find('.text').simulate("change", { target: { value: "foo" }});
        expect(wrapper.find('.Leaf').at(0).text()).toEqual(expect.stringContaining("foo"));
            });
    ...
    it('should edit value properly to an existing map', function(){
        ...
        const wrapper=mount(<Map initialState={{root:data0, property:prop}} />);
        wrapper.find('.Leaf').at(0).find('.text').simulate("change", { target: { value: "foo" }});
        expect(wrapper.find('.Leaf').at(0).text()).toEqual(expect.stringContaining("foo"));
        expect(wrapper.find('.Leaf').at(1).text()).toEqual(expect.stringContaining("second"));
    });
