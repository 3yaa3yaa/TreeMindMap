...
class StateProvider {
  ...
  // Reducer
  static leafReducer(
    state = { root: new LeafData(), property: new Property() },
    action
  ) {
    try {
      let result;
      if (
        state.property.isReadOnly === Property.readOnlyLevel().canEdit ||
        action.type === "changeMode" ||
        action.type === "changePreviewMode"
      ) {
        switch (action.type) {
          case "delete":
            result = StateProvider.delete(
              state,
              action.id,
              state.property.focusId
            );
            break;
          case "addRoot":
            result = StateProvider.addRoot(
              state,
              state.property.focusId,
              state.property.focusId
            );
            break;
          case "addSibling":
            let youngerbrothers = state.root.getYoungerBrother(
              state.property.focusId
            );
            if (youngerbrothers === null) {
              result = StateProvider.addSibling(state, action.id);
            } else {
              result = StateProvider.walk(
                state,
                state.property.focusId,
                StateProvider.whereToMove().DOWN
              );
            }
            break;
          case "addChild":
            ...
            if (children.length === 0) {
              ...
              result = StateProvider.walk(
                state,
                state.property.focusId,
                StateProvider.whereToMove().LEVELDOWN
              );
            }
            break;
          case "edit":
            result = StateProvider.edit(
              state,
              action.leaf,
              state.property.focusId
            );
  ...
  static move(state, from, to) {
    ...
    return {
      root: LeafData.getNewObject(state.root),
  ...
  static mapDispatchToProps(dispatch) {
    return {
      ...
      changePreviewMode: (mode) =>
        dispatch(StateProvider.changePreviewModeAction(mode)),
    };
  }
}
