...
export default class PreviewSentence extends Component {
  ...
  fillContents(leafdata, depth, out) {
    ...
    leafdata.children.forEach((child) =>
      this.fillContents(child, depth + 1, out)
    );
  }
  ...
  getTag(depth, leafdata) {
    ...
    if (
      outputdata.children.length > 0 &&
      depth < 7 &&
      !this.firstlineHasSpecialCharacter(outputdata.description)
    ) {
    ...
    return (
      <div
        key={leafdata.id}
        className="PreviewSentence-Paragraph"
  ...
  render() {
    return (
      <div className="PreviewSentence" style={{ marginTop: "10px" }}>
        {this.getContent()}
      </div>
    );
  }
