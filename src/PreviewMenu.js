...
export default class PreviewMenu extends Component {
  ...
  getCommandButton(previewMode, buttonimg, text, onClick) {
    ...
    return (
      <li className="PreviewMenu-button">
        <label
          htmlFor={id}
          className="ImageMenu-Label"
          style={this.getCommandLabelStyle(previewMode)}
        >
  ...
  getButtons() {
    return (
      <ul className="PreviewMenu-buttons">
        {this.getCommandButton(
          Property.previewMode().none,
          backToEditImg,
          "Back",
          () => {
  ...
  render() {
    return <div className="PreviewMenu">{this.getButtons()}</div>;
  }
}
