...
class Tree extends Component {
  ...
  _getConnector(leaf) {
    ...
    return (
      <Connector
        leafid={leaf.id}
        mode={this._getMode(this.props.root, leaf.id)}
      />
    );
  }
      ...
      dataarr.forEach((leaf) => {
        out.push(
          ...
          >
            ...
            >
              <ul
                key={leaf.id + "-leaf-element"}
                className="Tree-Element"
                style={{ margin: "0px" }}
              >
                <li
                  key={leaf.id + "-leaf-detail"}
                  className="Tree-Trunk-Sub"
                  style={{ margin: "0px" }}
                >
                  {this._getLeaf(leaf)}
                </li>
  ...
  render() {
    return (
      <DndProvider backend={HTML5Backend}>
        <div className="Tree">{this._getTree()}</div>
      </DndProvider>
    );
  }
}
