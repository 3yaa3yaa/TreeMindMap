...
export default class PreviewList extends Component {
  ...
  getHeader() {
    ...
    array = array.map((el, index) => {
      return (
        <div key={`header-${index}`} style={this.GetPreviewListCellStyle()}>
          {el}
        </div>
      );
    });
  ...
  getContent() {
    ...
    return array.map((item, rownum) => {
      let cells = [
        <MarkdownTextBox
          value={this.removeLabelAndFunction(item.description)}
          focus={false}
        />,
      ];
      ...
      cells = cells.map((el, colnum) => {
        return (
          <div
            key={`val-${rownum}-${colnum}`}
      ...
      return (
        <div key={`val-${rownum}`} style={this.GetPreviewListRowStyle()}>
          {cells}
        </div>
      );
    });
  ...
  GetPreviewListHeaderStyle() {
    return { display: "table-header-group", fontWeight: "bold" };
  }
  ...
  getContentOrInstruction() {
    if (this.leafdata.isNullObject()) {
      ...
      return (
        <div style={this.GetPreviewListStyle()}>
          {this.getHeader()}
          {this.getContent()}
        </div>
      );
